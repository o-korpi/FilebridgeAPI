...
class UserService(private val environment: ApplicationEnvironment) {
    ...
    private fun userKey(user: String) =
        "user:$user" // getEnvProperty(environment, "redis.keySchema.user") + user
    ...
    fun createUser(userCredentials: UserCredentials): HttpStatusCode {
        ...
        t.hset(
            ...
            mapOf("email" to userCredentials.email, "password" to hashedPassword))
    ...
    fun loginUser(userCredentials: UserCredentials): HttpStatusCode {
        val hashedPassword: String =
            db.hget(userKey(userCredentials.email), "password") ?: return HttpStatusCode.NotFound
        val passwordMatch = Hasher.verify(userCredentials.password, hashedPassword)
        if (!passwordMatch) return HttpStatusCode.Unauthorized
    ...
    fun updatePassword(userCredentials: UserCredentials) {
        ...
        db.hset(userKey(userCredentials.email), "password", hashedPassword)
    }

    fun deleteUser(userEmail: String): HttpStatusCode {
        val userKey = userKey(userEmail)

        if (!db.exists(userKey)) return HttpStatusCode.NotFound
