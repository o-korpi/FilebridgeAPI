...
class FileService(private val environment: ApplicationEnvironment) {
    ...
    private fun fileKey(fileId: String) =
        "file:$fileId" // getEnvProperty(environment, "redis.keySchema.file") + fileId

    private fun ownerKey(email: String) =
        "owner:$email" // getEnvProperty(environment, "redis.keySchema.owner") + email
    ...
    fun persistFile(file: FileUpload, owner: String): UUID? {
        ...
        runCatching {
                ...
                transaction.hset(fileKey, mapOf("name" to file.fileName, "data" to file.data))
                transaction.exec()
    ...
    fun getFileContent(fileId: String): FileContent? {
        ...
        if (name == "nil" || data == "nil") return null

        return FileContent(name, data)
    }
    ...
    fun getOwnedFiles(owner: String): List<FileData> {
        ...
        return fileKeys
            .mapNotNull { key ->
                ...
                FileData(fileId, fileName, remainingTime.toInt())
            }
